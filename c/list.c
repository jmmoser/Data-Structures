...
list_node *list_node_create(void *value, int element_size, deallocator dealloc) {
...
list_node *list_node_free(list_node *node) {
    ...
    if (node->dealloc && node->value) {
        node->dealloc(node->value);
    }
    free(node->value);
    free(node);
    return next;
...
int list_add(list *l, int index, void *value, int element_size, deallocator dealloc) {
    list_node *new_node = list_node_create(value, element_size, dealloc);
    list_node *node = 0;
    l->count++;
